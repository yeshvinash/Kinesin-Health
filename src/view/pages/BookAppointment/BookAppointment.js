...
import { CardImg, Col, FloatingLabel, Form, Image, Row } from "react-bootstrap";
...
export const BookAppointment = () => {
  ...
  const [showAlertStepIndex, setShowAlertStepIndex] = useState(-1);
  const [selectedDiv, setSelectedDiv] = useState(null);
  const [current, setCurrent] = useState(0);
  const onChange = (value) => {
    console.log("onChange:", value);
    setCurrent(value);
    setShowAlertStepIndex(current === 1 ? current : -1);
  };
  ...
  useEffect(() => {
    ...
    const hideAlert = setTimeout(() => {
      setShowAlert(false);
    }, 3000); // Hide after 5 seconds (adjust as needed)
      ...
      content: (
        <>
          <div className="authentication-title-wrap space-bottom">
            ...
            <div className="d-flex align-items-center pick-service-wrap">
              <span
                className={`pickup ${
        ...
        <>
          ...
          <Form>
            <div className="d-flex justify-content-between mb-4 pb-2">
              ...
              <Link to="" className="add-dependant-link">
